...
def create_source(name: str) -> Any:
    normalized_name = str(name).strip().lower()
    factory = SOURCE_FACTORIES.get(normalized_name)
...
def create_sources(
    ...
    return [create_source(name) for name in names]


def registered_sources() -> list[str]:
    return list(SOURCE_FACTORIES.keys())
